...
void bindControlSocket(ConfigManager& manager, std::string path) {
    ...
    if (bind(sockfd, (sockaddr*)&servaddr, sizeof(servaddr)) != 0) {
        LOG ("failed to bind control socket"); abort();
    }
    ...
    OsSocket::bindCustomFd(sockfd, [sockfd, &manager]() {
        ...
        if (response.size() > 0) {
            ...
            if (ret != response.size()) LOG("failed to write control packet response");
        }
        close(fd);
    });
}
...
void dropCap(std::string configDir) {
    ...
    if (chroot(configDir.c_str()) < 0) { perror("chroot"); exit(1); }
    if (chdir("/") < 0) { perror("chdir"); exit(1); }
    configDir = "/";

    if (prctl(PR_SET_SECUREBITS, SECBIT_KEEP_CAPS | SECBIT_NOROOT) < 0) { perror("prctl"); exit(1); }
    if (set_cap(0) < 0) { perror("set_cap"); exit(1); }
}
...
void iptablesInsert(std::string rule) {
    if (system(("ip6tables --check " + rule + "|| ip6tables --append " + rule).c_str()) != 0) {
...
std::string prepareConfigDir() {
  ...
  if (confdir == nullptr) confdir = "/var/lib/husarnet/";
...
void serviceMain(bool doFork=false) {
    ...
    if (system("modprobe ip6_tables 2>/dev/null; modprobe tun 2>/dev/null") != 0) {
        // ignore
    }
    ...
    if (identity->deviceId == BadDeviceId) { // we had a bug where bad IDs would be generated
        FileStorage::generateAndWriteId(configDir);
        identity = FileStorage::readIdentity(configDir);
    }
    ...
    configManager.shouldSendInitRequest = getenv("HUSARNET_SEND_INIT_REQUEST") != nullptr;

    if(system("[ -e /dev/net/tun ] || (mkdir -p /dev/net; mknod /dev/net/tun c 10 200)") != 0) {
    ...
    if (useTap) {
        ...
        if (system("ip link set hnet0 address 52:54:00:fc:94:4c") != 0 ||
            system(("ip neighbour add " + gateway + " lladdr " + macAddr + " dev hnet0 nud permanent").c_str()) != 0 ||
        ...
        // In future, we would like to have internal fragmentation support or PMTU discovery.
    ...
    if (system("ip -6 route add ff15:f2d3:a389::/48 dev hnet0 table local") != 0) {
