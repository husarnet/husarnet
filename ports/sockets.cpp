...
InetAddress ipFromSockaddr(struct sockaddr_storage st) {
    if (st.ss_family == AF_INET) {
        ...
        if (r.ip.data[0] == 0 && r.ip.data[1] == 0x64 &&
            r.ip.data[2] == 0xff && r.ip.data[3] == 0x9b) {
...
int bindUdpSocket(InetAddress addr, bool reuse, bool v6) {
    ...
    if (reuse) {
        int one = 1;
        SOCKFUNC(setsockopt)(fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&one, sizeof(one));
    }
...
bool udpListenMulticast(InetAddress address, PacketCallack callback) {
    if (address.ip.isMappedV4()) {
        int fd = bindUdpSocket(InetAddress{IpAddress(), (uint16_t)address.port}, true, /*ipv6=*/false);
        if (fd == -1) return false;
        multicastUdpFd4 = fd;
        ...
        if (SOCKFUNC(setsockopt)(fd, IPPROTO_IP, IP_ADD_MEMBERSHIP,
        ...
        int fd = bindUdpSocket(InetAddress{IpAddress(), (uint16_t)address.port}, true);
        if (fd == -1) return false;
        multicastUdpFd6 = fd;
        ...
        if (SOCKFUNC(setsockopt)(fd, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
                (const char*)&mreq, sizeof(mreq)) == 0) {
            udpSockets.push_back(UdpSocket{fd, callback});
            return true;
        } else {
...
void udpSendMulticast(InetAddress address, const std::string& data) {
    ...
    if (address.ip.isMappedV4()) {
        if (multicastUdpFd4 == -1) return;
        SOCKFUNC(sendto)(multicastUdpFd4, data.data(), data.size(), 0, (sockaddr*)&sa, socklen);
    } else {
        if (multicastUdpFd6 == -1) return;
        SOCKFUNC(sendto)(multicastUdpFd6, data.data(), data.size(), 0, (sockaddr*)&sa, socklen);
    }
}
...
bool write(std::shared_ptr<FramedTcpConnection> conn, const std::string& packet, bool doQueue) {
    if (conn->fd == -1) return false;
...
void writeQueue(std::shared_ptr<FramedTcpConnection> conn) {
    ...
    long wrote = SOCKFUNC(send)(conn->fd,
                                 conn->queue.data() + conn->queuePtr,
...
void handleRead(std::shared_ptr<FramedTcpConnection> conn) {
    while (true) {
        ...
        long rd = 0;
        if (readSize != 0) {
            rd = SOCKFUNC(recv)(conn->fd,
                                (char*)conn->readBuffer.data() + conn->readBufferPtr,
                                readSize, 0);
        ...
        while (true) {
            ...
            if (conn->readBufferPtr <= 5) return;
            ...
            if (expectedSize + 5 >= (int)conn->readBuffer.size()) {
                LOG("TCP message too large (%d, max is %d)", expectedSize, (int)conn->readBuffer.size());
                close(conn); // oops
                return;
            }
            ...
            if (expectedSize + 5 <= conn->readBufferPtr) {
                ...
                memmove(&conn->readBuffer[0], &conn->readBuffer[newPtr],
...
std::shared_ptr<FramedTcpConnection> tcpConnect(InetAddress address,
    ...
    int val = 1;
    SOCKFUNC(setsockopt)(conn->fd, IPPROTO_TCP, TCP_NODELAY, (const char*)&val, sizeof(int));
...
void runOnce(int timeout) {
    ...
    GIL::unlocked<void>([&]() {
        ...
        int res = SOCKFUNC(select)(maxfd + 1, &readset, &writeset, &errorset, &timeoutval); // ignore result
        (void)res;
    });
    ...
    for (auto& conn : udpSockets) {
        ...
        if (FD_ISSET(conn.fd, &readset)) {
            ...
            while (true) {
                long r = SOCKFUNC(recvfrom)(conn.fd, &udpBuffer[0], udpBuffer.size(), 0,
                    (sockaddr*)&s, &len);
                if (r <= 0) break;
