...
void NotificationManager::refreshNotificationFile(){
    //This ma look a bit confusing but in order to solve several edge cases with cutom time and date settings on client systems
    //and in order to handle data loss as gracefully as possible this kind of solutions was the best apparant one.
    // We expect to get new announcements for dashbaord in a json contatining a list of objects, where each object has id, 
    //content of the announcement and a valididty span (in days). On the client side we store the the latest_id we got 
    //(and when looking at announcements we got from dashbaord we igonre those with lower or equal id to this one), 
    //we also store announcements that are still walid and should be displayed 
    //(we store content of the announcement and a time point casted to int once the time point is excedeed this announcement will be filtered out on refresh)
    //we utilze client system clock to calculate the validity time point adn only specify timespan on the server side
    //this mechanism ensures that even if client system is configured with a custom date and time, 
    //announcements will work as if the time and date was the same as on the server side. 
    //The id mechanism also ensures that individual announcements will be displayed to users for the same time spans 
    //independently of the time client received announcement.
    ...
    if(cached!="{}" and new_notifications.contains("announcements")){
        ...
        for(const auto& element : new_notifications["announcements"]){
            if(element["id"]>merged["latest_id"]){
                ...
                j.emplace("valid_until", now+element["valid_for"].get<int>()*86400/*seconds in days*/);
