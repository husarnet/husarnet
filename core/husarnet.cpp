...
struct PeerToBaseMessage {
    enum { REQUEST_INFO, DATA, INFO, NAT_INIT, USER_AGENT, NAT_OK_CONFIRM, NAT_INIT_TRANSIENT, INVALID } kind;
...
struct NgSocketImpl : public NgSocket {
    ...
    // We send a packet to one port from a range of base ports (around 20 ports), so we get new NAT
    // mapping every time. (20 * 25s = 500s and UDP NAT mappings typically last 30s-180s).
    // This exists to get around permanently broken NAT mapping (which may occur due to
    // misconfiguration or Linux NAT implementation not always assigning the same outbound
    // port)
    ...
    void periodic() override {
        if (currentTime() < lastPeriodic + 1000) return;
        ...
        if (!natInitConfirmed && currentTime() - lastNatInitSent > NAT_INIT_TIMEOUT) {
            // retry nat init in case the packet gets lost
            sendNatInitToBase();
        }

        if (currentTime() - lastBaseTcpAction > (baseConnection ? TCP_PONG_TIMEOUT : NAT_INIT_TIMEOUT))
            connectToBase();
    }

    void resolveBaseAddress() {
        while (!baseAddressFromDns) {
            baseAddressFromDns = GIL::unlocked<IpAddress>([&]() { return resolveToIp(baseConfig->getBaseDnsAddress()); });
        ...
        while (true) {
            IpAddress result = GIL::unlocked<IpAddress>([&]() { return resolveToIp(baseConfig->getBaseDnsAddress()); });
            LOG("base address resolved to: %s", result.str().c_str());
            if (result) baseAddressFromDns = result;
    ...
    std::string peerInfo(DeviceId id) override {
        ...
        if (!peer) return "";
        if (!peer->active() && !peer->reestablishing) return "";
    ...
    void periodicPeer(Peer* peer) {
        if (!peer->active()) {
            ...
            // TODO: send unsub to base
        } else {
            if (peer->reestablishing && peer->connected && currentTime() - peer->lastReestablish > REESTABLISH_TIMEOUT) {
                peer->connected = false;
    ...
    void sendDataToPeer(Peer* peer, string_view data) {
        if (peer->connected) {
            ...
            if (!peer->reestablishing || (currentTime() - peer->lastReestablish > REESTABLISH_TIMEOUT && peer->failedEstablishments <= MAX_FAILED_ESTABLISHMENTS))
                attemptReestablish(peer);
    ...
    Peer* createPeer(DeviceId id) {
        if (id == deviceId) return nullptr; // self
    ...
    void attemptReestablish(Peer* peer) {
        ...
        LOGV("reestablish connection to [%s]", IpAddress::fromBinary(peer->id).str().c_str());
        ...
        for (InetAddress address : addresses) {
            if (address.ip.isFC94()) continue;
            if (!options->isAddressAllowed(address)) continue;
    ...
    void helloReceived(InetAddress source, const PeerToPeerMessage& msg) {
        if (msg.yourId != deviceId) return;

        Peer* peer = getOrCreatePeer(msg.myId);
        if (peer == nullptr) return;
        LOGV("HELLO from %s (id: %s, active: %s)", source.str().c_str(), IDSTR(msg.myId), peer->active() ? "YES" : "NO");
        ...
        if (source.ip.isLinkLocal() && !peer->linkLocalAddress){
            ...
            if (peer->active()) attemptReestablish(peer);
        }
    ...
    void helloReplyReceived(InetAddress source, const PeerToPeerMessage& msg) {
        ...
        LOGV("HELLO-REPLY from %s (%s)", source.str().c_str(), encodeHex(msg.myId).c_str());
        Peer* peer = getPeerById(msg.myId);
        if (peer == nullptr) return;
        if (!peer->reestablishing) return;
        if (peer->helloCookie != msg.helloCookie) return;
    ...
    void baseMessageReceivedTcp(const BaseToPeerMessage& msg) {
        ...
        if (msg.kind == BaseToPeerMessage::HELLO) {
            ...
            if (!options->disableUdp) {
                ...
                if (peer != nullptr) changePeerTargetAddresses(peer, msg.addresses);
            }
        } else if (msg.kind == BaseToPeerMessage::DATA) {
            delegate->onDataPacket(msg.source, msg.data);
    ...
    void sendMulticast() {
        if (options->disableUdp || options->disableMulticast) return;
    ...
    void multicastPacketReceived(InetAddress address, string_view packetView) {
        ...
        LOGV("multicast received from %s, id %s, port %d, interesting: %s", address.str().c_str(), IDSTR(devId), port, peer == NULL ? "NO" : "YES");
    ...
    std::vector<Peer*> iteratePeers() {
        ...
        for (auto& p : peers) peersVec.push_back(p.second);
        return peersVec;
    }
};
...
BaseToPeerMessage parseBaseToPeerMessage(string_view data) {
    ...
    if(data.size() == 0) return msg;

    if (data[0] == (char)BaseToPeerMessage::HELLO) {
        if (data.size() != 17) return msg;
        ...
        while (i + 18 <= data.size() && msg.udpAddress.size() < 5) {
            msg.udpAddress.push_back(
                InetAddress{IpAddress::fromBinary(data.substr(i, 16)), unpack<uint16_t>(data.substr(i + 16, 2))});
            i += 18;
        }

        if (i + 4 <= data.size()) {
            msg.natTransientRange = {unpack<uint16_t>(data.substr(i, 2)),
                                     unpack<uint16_t>(data.substr(i + 2, 2)),};
        }

        if (msg.udpAddress.size() < 1) return msg;
    } else {
        return msg;
    }
...
PeerToPeerMessage parsePeerToPeerMessage(string_view data) {
    ...
    if (data[0] == (char)PeerToPeerMessage::HELLO || data[0] == (char)PeerToPeerMessage::HELLO_REPLY) {
        if (data.size() != 1 + 16 * 3 + 32 + 64) return msg;
        ...
        bool ok = GIL::unlocked<bool>([&]() {
                return verifySignature(data.substr(0, 17 + 64), "ng-p2p-msg", pubkey, signature); });
...
std::string NgSocketImpl::serializePeerToPeerMessage(const PeerToPeerMessage& msg) {
    std::string data;

    if (msg.kind == PeerToPeerMessage::HELLO || msg.kind == PeerToPeerMessage::HELLO_REPLY) {
        assert(deviceId.size() == 16 && msg.yourId.size() == 16 && msg.helloCookie.size() == 16
               && pubkey.size() == 32);
        data = pack((uint8_t)msg.kind) + deviceId + pubkey + msg.yourId + msg.helloCookie;
        data += GIL::unlocked<std::string>([&]() { return sign(data, "ng-p2p-msg"); });
...
std::string NgSocketImpl::serializePeerToBaseMessage(const PeerToBaseMessage& msg) {
    assert(deviceId.size() == 16 && cookie.size() == 16 && pubkey.size() == 32);
    std::string data = pack((uint8_t)msg.kind) + this->deviceId + this->pubkey + this->cookie;
...
void NgSocketImpl::connectToBase() {
    ...
    if (options->overrideBaseAddress) {
        ...
        if (baseConnectRetries > 2) {
            ...
            baseAddress = baseTcpAddressesTmp[baseConnectRetries % baseTcpAddressesTmp.size()];
    ...
    baseConnection = tcpConnect(baseAddress,
...
void NgSocketImpl::sendToBaseUdp(const PeerToBaseMessage& msg) {
    if (!baseConnection || cookie.size() == 0) return;
