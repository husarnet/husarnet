...
struct UnwrapperImpl : NgSocket, NgSocketDelegate {
    ...
    void sendDataPacket(DeviceId target, string_view packet) override {
        ...
        // LOG("to: %s (exp: %s), from: %s (exp: %s), proto: %s", encodeHex(packet.substr(0, 6)).c_str(), encodeHex(peerMacAddr).c_str(), encodeHex(packet.substr(6, 6)).c_str(), encodeHex(selfMacAddr).c_str(),
        //    encodeHex(packet.substr(12, 2)).c_str());

        if (packet.substr(0, 6) == peerMacAddr && packet.substr(6, 6) == selfMacAddr && packet.substr(12, 2) == string_view("\x86\xdd", 2)) {
