...
bool ConfigManager::isPeerAllowed(DeviceId id) {
  ...
  auto ret =
      configTable->getValue("whitelist", IpAddress::fromBinary(id).str());
  return ret.size() > 0;
}
...
std::string ConfigManager::configGet(std::string networkId,
                                     std::string key,
...
void ConfigManager::configSet(std::string networkId,
                              std::string key,
                              std::string value) {
...
void ConfigManager::joinNetwork(std::string joinCode, std::string hostname) {
  ...
  configTable->runInTransaction([&]() {
    configTable->insert(
        ConfigRow{"manual", "whitelist", joinInfo.first.str(), "1"});
    configTable->insert(
        ConfigRow{"manual", "manager", joinInfo.first.str(), "1"});
...
std::string ConfigManager::handleControlPacket(std::string data) {
  ...
  if (cmd != "status" && cmd != "status-json" && cmd != "whitelist-ls" &&
      cmd != "logs" && cmd != "get-verbosity" && cmd != "logs-size" &&
      cmd != "logs-current")
  ...
  try {
    if (cmd == "whitelist-add") {
      ...
      configTable->insert(ConfigRow{"manual", "whitelist",
                                    IpAddress::fromBinary(addr).str(), "1"});
      ...
      if (addr.size() != 16)
        return "fail";
      configTable->remove(ConfigRow{"manual", "whitelist",
                                    IpAddress::fromBinary(addr).str(), "1"});
      ...
      std::vector<ConfigRow> values =
          configTable->listValuesForNetwork("manual", "whitelist");
      std::string response = "";
      ...
      if (parts.size() == 2) {
        ConfigRow row{"manual", "host", parts[1],
                      IpAddress::parse(parts[0]).str()};
        if (cmd == "host-add")
      ...
      reader.exceptions(std::istringstream::failbit |
    ...
    } else if (cmd == "get-websetup-token") {  // These two commands are used
                                               // internally to i.e. join a
                                               // network. There's no need for
                                               // exposing them to users.
      for (const std::string& websetupHost :
           baseConfig->getDefaultWebsetupHosts())
        configTable->insert(ConfigRow{
            "manual", "whitelist", IpAddress::parse(websetupHost).str(), "1"});
...
std::string ConfigManager::handleWebsetupPacket(InetAddress addr,
                                                std::string data) {
  std::string sharedSecret = getLegacySharedSecret();

  if (addr.ip &&
      configTable->getValueForNetwork("manual", "manager", addr.str()).size() >
          0) {
  ...
  try {
    if (cmd == "whitelist-rm" || cmd == "whitelist-add" ||
        cmd == "whitelist-enable" || cmd == "whitelist-disable" ||
        cmd == "host-add" || cmd == "host-rm") {
      ...
      configTable->runInTransaction([&]() {
        ...
        for (ConfigRow row :
             configTable->listValuesForNetwork("manual", "manager"))
          configTable->insert(
              ConfigRow{"manual", "whitelist", row.key, row.value});
      });
      ...
      return (seqnum + ("version=" HUSARNET_VERSION ";ua=") +
              sock->options->userAgent);
    } else {
...
void ConfigManager::sendWebsetupUdp(InetAddress address, std::string body) {
  ...
  int ret = SOCKFUNC(sendto)(websetupFd, body.data(), body.size(), 0,
                             (sockaddr*)&addr, addrsize);
  if (ret < 0)
    LOG("websetup UDP send failed (%d)", (int)errno);
}

std::pair<IpAddress, std::string> ConfigManager::getJoinInfo(
    std::string joinCode) {
...
void ConfigManager::sendInitRequest() {
  ...
  for (const std::string& host : baseConfig->getDefaultWebsetupHosts()) {
    ...
    configTable->insert(
        ConfigRow{"manual", "whitelist", IpAddress::parse(host).str(), "1"});
  }

  if (this->joinCode.size() > 0) {
    auto joinInfo = getJoinInfo(this->joinCode);
    LOG("sending join code request: [%s] [%s] [%s]",
        joinInfo.first.str().c_str(), joinInfo.second.c_str(),
        joinCode.c_str());
    configTable->insert(
        ConfigRow{"manual", "whitelist", joinInfo.first.str(), "1"});
  ...
  while (true) {
    if (shouldSendInitRequest) {
      if (!initResponseReceived && currentTime() - lastSent > 1000) {
        ...
        // We send 'init request' to all saved servers every few seconds, until
        // we get response from any of them. This is disabled by default in
        // standard Husarnet daemon, because its whitelist and hosts DB is
        // persistent.
        sendInitRequest();
      }
    }
    ...
    int ret = GIL::unlocked<int>([&]() {
      return SOCKFUNC(recvfrom)(websetupFd, &buffer[0], buffer.size(), 0,
                                (sockaddr*)&addr, &addrsize);
    });
...
void ConfigManager::httpThread() {
  ...
  svr.Post("/control/reset-received-init-response",
           [&](const httplib::Request& req, httplib::Response& res) {
  ...
  svr.Post("/control/logs/verbosity",
           [&](const httplib::Request& req, httplib::Response& res) {
  ...
  svr.Post("/control/has-received-init-response",
           [&](const httplib::Request& req, httplib::Response& res) {
             ...
             initResponseReceived ? res.set_content("yes", "text/plain")
    ...
    try {
      if (req.has_param("address")) {
        ...
        if (addr.size() != 16) {
          ...
          configTable->insert(ConfigRow{
              "manual", "whitelist", IpAddress::fromBinary(addr).str(), "1"});
          res.set_content("ok", "text/plain");
        }
      } else {
        res.set_content("fail", "text/plain");
