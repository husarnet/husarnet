...
{
  LOG_DEBUG(logger, "received data packet from peer // {peer}", peerId.toString());
  ...
  if(!peer->negotiated) {
    ...
    LOG_WARNING(logger, "received data packet before hello // {peer}", peerId.toString());
    return;
  }
  ...
  if(r == 0) {
    ...
    LOG_INFO(logger, "received forged message from peer // {peer}", peerId.toString());
  }
}
...
void SecurityLayer::handleHelloPacket(HusarnetAddress target, string_view data, int helloNum)
{
  ...
  if(data.size() < dataLen + 64)
    return;
  LOG_INFO(logger, "handle hello packet // {peer} {hello_num}", target.toString(), helloNum);
  ...
  LOG_DEBUG(logger, "peer flags // {peer} {flags}", target.toString(), (unsigned long long)flags_bin);

  if(targetId != this->myIdentity->getDeviceId()) {
    LOG_INFO(logger, "misdirected hello packet received // {peer}", std::string(targetId));
    return;
  }

  if(NgSocketCrypto::pubkeyToDeviceId(incomingPubkey) != target) {
    LOG_INFO(logger, "forged hello packet received (invalid pubkey) // {pubkey}", std::string(incomingPubkey));
    return;
  }

  if(!NgSocketCrypto::verifySignature(data.substr(0, data.size() - 64), "ng-kx-pubkey", incomingPubkey, signature)) {
    LOG_CRITICAL(logger, "forged hello packet (invalid signature) // {signature}", std::string(signature));
    return;
  }
  ...
  if(myHelloseq != this->helloseq) {  // prevents replay DoS
    ...
    LOG_DEBUG(logger, "invalid helloseq // {peer}", peer->getIpAddressString());
    return;
  }
...
{
  LOG_INFO(logger, "established secure connection // {peer}", peer->getIpAddressString());
