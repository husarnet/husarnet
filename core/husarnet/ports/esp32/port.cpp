...
static std::string readNVS(const std::string& path)
{
  ...
  if(err != ESP_OK) {
    HUSARNET_LOG_ERROR("Unable to access NVS. (Error: %s)", esp_err_to_name(err));
    return "";
  }
...
static bool writeNVS(const std::string& path, const std::string& data)
{
  ...
  if(err != ESP_OK) {
    HUSARNET_LOG_ERROR("Unable to update NVS. (Error: %s)", esp_err_to_name(err));
    return false;
  }

  if(nvsHandle->commit() != ESP_OK) {
    HUSARNET_LOG_ERROR("Unable to commit NVS. (Error: %s)", esp_err_to_name(err));
    return false;
  }
...
namespace Port {
  ...
  void init()
  {
    ...
    if(err == ESP_ERR_NVS_NOT_INITIALIZED) {
      HUSARNET_LOG_INFO("NVS not initialized. Initializing...");
      err = nvs_flash_init();

      // NVS partition was truncated and needs to be erased
      // Erase and retry init
      if(err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        HUSARNET_LOG_INFO("Erasing NVS partition...");
        err = nvs_flash_erase();
        if(err != ESP_OK) {
          HUSARNET_LOG_CRITICAL("Unable to erase NVS. (Error: %s)", esp_err_to_name(err));
          abort();
        }

        err = nvs_flash_init();
        if(err != ESP_OK) {
          HUSARNET_LOG_CRITICAL("Unable to reinitialize NVS. (Error: %s)", esp_err_to_name(err));
          abort();
        }
      }

      else if(err != ESP_OK) {
        HUSARNET_LOG_CRITICAL("Unable to initialize NVS. (Error: %s)", esp_err_to_name(err));
        abort();
      }
    ...
    if(err != ESP_OK) {
      HUSARNET_LOG_ERROR("Unable to open NVS. (Error: %s)", esp_err_to_name(err));
      abort();
    }
    ...
    if(notifyReadySemaphore == NULL) {
      HUSARNET_LOG_CRITICAL("Unable to create semaphore");
      abort();
    }
  }
  ...
  {
    HUSARNET_LOG_CRITICAL("Fatal error: %s", msg.c_str());
    abort();
  }
  ...
  void threadStart(std::function<void()> func, const char* name, int stack, int priority)
  {
    ...
    *func1 = std::move(func);
    if(xTaskCreate(taskProc, name, stack, func1, priority, NULL) != pdTRUE) {
      HUSARNET_LOG_CRITICAL("Unable to create task");
      abort();
    }
  }
  ...
  {
    HUSARNET_LOG_ERROR("getIpAddressFromInterfaceName is not implemented");
    return IpAddress();
  }
  ...
  static esp_err_t _getLocalAddressesTcpipFunc(void* ctx)
  {
    ...
    for(u8_t idx = 1; (netif = netif_get_by_index(idx)) != NULL; idx++) {
      ...
      for(int i = 0; i < LWIP_IPV6_NUM_ADDRESSES; i++) {
        ...
        HUSARNET_LOG_DEBUG("netif: %s, ip6: %s", netif->name, buf);
      ...
      HUSARNET_LOG_DEBUG("netif: %s, ip4: %s", netif->name, buf);
  ...
  std::vector<IpAddress> getLocalAddresses()
  {
    ...
    if(res != ESP_OK) {
      HUSARNET_LOG_ERROR("Failed to get local addresses");
    }
  ...
  {
    HUSARNET_LOG_ERROR("runScripts not supported on ESP32");
    return false;
  }
  ...
  {
    if(ip.isInvalid()) {
      HUSARNET_LOG_ERROR("Invalid ip address");
      return {-1, ""};
    }
    ...
    if(!message.encode(buffer)) {
      HUSARNET_LOG_ERROR("Failed to encode HTTP message");
      return {-1, ""};
    }
    ...
    if(sockfd < 0) {
      HUSARNET_LOG_ERROR("can't contact %s - is DNS resolution working properly?", ip.toString().c_str());
      return {-1, ""};
    }
    ...
    do {
      ...
      if(n <= 0) {
        HUSARNET_LOG_ERROR("Failed to send HTTP request");
    ...
    if(n <= 0) {
      HUSARNET_LOG_ERROR("Failed to receive HTTP response");
      SOCKFUNC(close)(sockfd);
      return {-1, ""};
    }
    ...
    HUSARNET_LOG_ERROR("Failed to parse HTTP response");
    return {-1, ""};
  }
  ...
  HttpResult httpGet(const std::string& host, const std::string& path)
  {
    ...
    if(ip.isInvalid()) {
      HUSARNET_LOG_ERROR("Can't resolve %s to IP", host.c_str());
      return {-1, ""};
    }
