...
static int openTun(std::string& name)
{
  ...
  while(unitNumber < 10) {
    ...
    if(connect(fd, (struct sockaddr*)&sc, sizeof(sc)) == -1) {
      LOG_WARNING(logger, "Can't bind to utun // {utun_num}", unitNumber - 1);
...
Tun::Tun()
{
  ...
  this->name = tunName;
  if(fd == -1) {
    LOG_ERROR(logger, "Can't open utun device!");
  } else {
    LOG_INFO(logger, "utun device opened successfully");
  }
...
void Tun::onLowerLayerData(HusarnetAddress source, string_view data)
{
  ...
  if(wr != wrapped.size()) {
    LOG_INFO(logger, "short tun write");
  }
}
