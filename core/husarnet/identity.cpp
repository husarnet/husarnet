...
Identity* Identity::init()
{
  ...
  if(id_string.empty()) {
    HLOG_WARNING("No identity found!");
  } else {
    identity = Identity::deserialize(id_string);
  }

  if(!identity->isValid()) {
    HLOG_WARNING("Identity is invalid, generating a new one");

    identity = Identity::create();

    auto success = Port::writeStorage(StorageKey::id, identity->serialize());
    if(!success) {
      HLOG_CRITICAL("Failed to save identity to storage, will run with a volatile one!");
    }
  }
