...
namespace NgSocketCrypto {
  ...
  bool
  verifySignature(const std::string& data, const std::string& kind, const fstring<32>& pubkey, const fstring<64>& sig)
  {
    std::string msg = makeMessage(data, kind);
    int ret = crypto_sign_ed25519_verify_detached(
        (unsigned char*)&sig[0], (const unsigned char*)msg.data(), msg.size(), (const unsigned char*)pubkey.data());
    return ret == 0;
  }
