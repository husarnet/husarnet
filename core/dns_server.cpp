...
struct DnsRequest {
    bool ok;
    uint16_t id;
    struct Label { std::string name; uint16_t qtype; uint16_t qclass; };

    std::vector<Label> labels;
};

DnsRequest parseDnsRequest(std::string s) {
    if (s.size() < 12) { LOG("DNS request too short"); return {false}; }
    ...
    for (int i=0; i < qcount; i ++) {
        ...
        LOG("query %s (qtype = %d, qclass = %d)", label.c_str(), (int)qtype, (int)qclass);
...
std::string makeDnsResponse(int id, ResolveResponse resp, DnsRequest::Label query) {
    uint16_t opcode = 0;
    uint16_t flags = (1 << 15) | (opcode << 12) | (int)resp.code
        | /* rd|ra */ (1<<7) | (1<<8);
    uint16_t qcount = (query.name.size() > 0) ? 1 : 0;
    uint16_t acount = (resp.code == ResolveResponse::OK && query.qtype == 28 && query.qclass == 1) ? 1 : 0;
    uint16_t nscount = 0;
    uint16_t arcount = 0;

    std::string body =
        pack(htons(id)) + pack(htons(flags)) + pack(htons(qcount)) +
        pack(htons(acount)) + pack(htons(nscount)) + pack(htons(arcount));

    if (qcount == 1) {
        body += mk_label(query.name);
        body +=
            pack(htons(uint16_t(query.qtype))) +
    ...
    if (acount == 1) {
        ...
        body +=
            pack(htons(uint16_t(28))) +
            pack(htons(uint16_t(1))) +
            pack(htonl(uint32_t(10))) +
            pack(htons(uint16_t(rdata.size())));
        body += rdata;
    }
...
std::string processDnsRequest(std::string s, ResolveFunc resolver) {
    ...
    if(req.labels.size() > 0) {
        ...
        if (resp.code == ResolveResponse::IGNORE) return "";
...
uint16_t checksum(std::string pkt) {
    ...
    int s = 0;
    for (int i=0; i < pkt.size(); i += 2) s += unpack<uint16_t>(pkt.substr(i, 2));
    s = (s >> 16) + (s & 0xffff);
    s += s >> 16;
    s = ~s;
    return s & 0xffff;
...
std::string makeUdp6Packet(std::string src, std::string dst, int srcport, int dstport, std::string data) {
    std::string pseudoheader = src + dst + pack(htons(data.size() + 8)) +
                   std::string("\0\0\0\x11", 4);
    std::string header = pack(htons(srcport)) + pack(htons(dstport)) + pack(htons(data.size() + 8)) + pack(htons(0));
    std::string chksum = pack(checksum(pseudoheader + header + data)); // native endian
...
std::string processUdpWithDnsRequest(std::string src, std::string dst, std::string data, ResolveFunc resolver) {
    if (data[0] != 0x11) return ""; // not udp
    UdpPacket packet = parseUdp6Packet(data.substr(1));
    if (packet.dst_port != 53) { return ""; }
...
struct DnsSocketWrapper : NgSocket, NgSocketDelegate {
    NgSocket* socket;
    const DeviceId dnsServerAddress = IpAddress::parse("fc94::1").toBinary();; //IpAddress::parse("2001:4860:4860::8888").toBinary();
    ...
    DnsSocketWrapper() {
        dnsFd = OsSocket::bindUdpSocket(InetAddress{IpAddress(), (uint16_t)0}, false);

        OsSocket::bindCustomDgramFd(dnsFd, std::bind(&DnsSocketWrapper::dnsReply, this, std::placeholders::_1, std::placeholders::_2));
    }

    void dnsReply(InetAddress src, string_view data) {
        LOG("dns reply: %s", encodeHex(data).c_str());
        if (src != recursiveDnsServer) return;
        ...
        delegate->onDataPacket(dnsServerAddress, '\x11' + makeUdp6Packet(dnsServerAddress, deviceId, 53, orgPort, newData));
    }
    ...
    void sendDataPacket(DeviceId target, string_view packet) override {
        if (target == dnsServerAddress) {
            if (packet.size() >= 8 && packet[0] == 0x3A && packet[1] == 0x80) { // ICMP
                // reply to pings
                std::string response;
                ...
                response +=
                    pack<uint16_t>(htons(
                        htons(unpack<uint16_t>(packet.substr(3, 2))) - 256));
                response += packet.substr(5);

                LOG("ping %s -> %s", encodeHex(packet).c_str(), encodeHex(response).c_str());
                delegate->onDataPacket(dnsServerAddress, response);
                return;
            }

            std::string resp = processUdpWithDnsRequest(deviceId, target, packet, resolver);
            if (resp == "") {
