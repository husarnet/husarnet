...
struct NgSocketOptions {
    ...
    std::function<bool(InetAddress)> isAddressAllowed = [](InetAddress) { return true; };
    std::function<std::string(DeviceId)> peerAddressInfo = [](DeviceId) { return ""; }; // only used in `info()`
    std::function<std::vector<InetAddress>(DeviceId)> additionalPeerIpAddresses = [](DeviceId) { return std::vector<InetAddress>{}; };
