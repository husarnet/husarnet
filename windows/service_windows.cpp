...
void handleControlConnection(ConfigManager& manager, int fd) {
    ...
    if (ret < 0) { LOG("failed to receive control packet"); return; }
...
void bindControlSocket(ConfigManager& manager) {
    sockaddr_in6 servaddr = OsSocket::sockaddrFromIp(InetAddress::parse("[::1]:5579"));

    int sockfd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); // why AF_INET6 doesn't work?
    if (bind(sockfd, (sockaddr*)&servaddr, sizeof(servaddr)) != 0) {
        LOG ("failed to bind control socket (err = %d)", WSAGetLastError()); abort();
    }
    if (listen(sockfd, SOMAXCONN) != 0) abort();

    startThread([sockfd, &manager]() {
        while (true) {
            int fd = accept(sockfd, nullptr, nullptr);
            if (fd < 0) { LOG("control socket accept failed"); return; }

            startThread([fd, &manager]() { handleControlConnection(manager, fd); }, "control", 0);
        }
    }, "control", 0);
}
...
void fixPermissions(std::string path) {
    ...
    system(("icacls \"" + path + "\" /grant:r *S-1-5-18:(F) *S-1-3-0:(F) *S-1-5-32-544:(F) /inheritance:r").c_str());
    system(("icacls \"" + path + "\" /grant:r *S-1-5-18:(OI)(CI)F *S-1-3-0:(OI)(CI)F *S-1-5-32-544:(OI)(CI)F /inheritance:r").c_str());
...
void serviceMain() {
    ...
    NgSocket* netDevL3 = NetworkDev::wrap(identity->deviceId, wrappedSock,
    ...
    if (netshName != sourceNetshName) {
        if (mySystem("netsh interface set interface name = \"" + sourceNetshName + "\" newname = \"" + netshName + "\"") == 0) {
    ...
    mySystem("netsh interface ipv6 add neighbors " + quotedName + " fc94:8385:160b:88d1:c2ec:af1b:06ac:0001 52-54-00-fc-94-4d");
    std::string myIp = IpAddress::fromBinary(identity->deviceId).str();
    mySystem("netsh interface ipv6 add address " + quotedName + " " + myIp + "/128");
    mySystem("netsh interface ipv6 add route fc94:8385:160b:88d1:c2ec:af1b:06ac:0001/128 " + quotedName);
