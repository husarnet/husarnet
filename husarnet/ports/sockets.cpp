...
bool udpListenMulticast(InetAddress address, PacketCallack callback) {
  if (address.ip.isMappedV4()) {
    int fd = bindUdpSocket(InetAddress{IpAddress(), (uint16_t)address.port},
                           true, /*ipv6=*/false);
    if (fd == -1)
      return false;
    multicastUdpFd4 = fd;
...
void writeQueue(std::shared_ptr<FramedTcpConnection> conn) {
  ...
  long wrote = SOCKFUNC(send)(conn->fd, conn->queue.data() + conn->queuePtr,
...
void runOnce(int timeout) {
  ...
  GIL::unlocked<void>([&]() {
    ...
    int res = SOCKFUNC(select)(maxfd + 1, &readset, &writeset, &errorset,
