...
void WebsetupConnection::send(std::string command,
  ...
  if (SOCKFUNC(sendto)(websetupFd, data, sizeof(data), 0, websetupAddr,
                       sizeof(*websetupAddr)) < 0) {
    LOG("websetup UDP send failed (%d)", (int)errno);
  }
}

void WebsetupConnection::sendJoinRequest(std::string joinCode,
...
std::list<std::string> WebsetupConnection::handleWebsetupCommand(
  ...
  if (command == "ping") {
    ...
    return {std::string("version=") + manager->getVersion() +
