...
void LegacyConfig::sqliteBind(sqlite3_stmt* stmt, std::vector<std::string> args) {
  ...
  for (int i = 0; i < args.size(); i++) {
    int res = sqlite3_bind_text(stmt, i + 1, args[i].data(), args[i].size(), SQLITE_TRANSIENT);
    if (res != SQLITE_OK) {
...
std::vector<std::string> LegacyConfig::sqliteIterate(sqlite3_stmt* stmt, std::function<std::string()> f) {
...
{
  auto getWebsetupSecretStatement = sqlitePrepareStatement("select value from websetup_data where kind=? and key=?;");
  sqliteBind(getWebsetupSecretStatement, {"config", "websetup-secret"});

  auto values = sqliteIterate(getWebsetupSecretStatement, [&]() { return sqliteGetValue(getWebsetupSecretStatement, 0); });
  return (values.size() > 0) ? values.at(0) : "";
}

bool LegacyConfig::getWhitelistEnabled()
{
  auto getWhitelistEnabledStatement = sqlitePrepareStatement("select value from websetup_data where kind=? and key=?;");
  sqliteBind(getWhitelistEnabledStatement, {"config", "whitelist-enabled"});

  auto values = sqliteIterate(getWhitelistEnabledStatement, [&]() { return sqliteGetValue(getWhitelistEnabledStatement, 0); });
...
{
  auto getWhitelistEntriesStatement = sqlitePrepareStatement("select key from websetup_data where kind=?;");
  sqliteBind(getWhitelistEntriesStatement, {"whitelist"});

  auto values = sqliteIterate(getWhitelistEntriesStatement, [&]() { return sqliteGetValue(getWhitelistEntriesStatement, 0); });
